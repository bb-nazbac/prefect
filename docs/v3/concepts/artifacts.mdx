...
When an artifact is created with a non-null `key`, the server automatically inserts or updates an `ArtifactCollection` entry to track this artifact as the latest version for that key. This means each key has a "latest" pointer that is updated whenever a new artifact is created with that key. The server verifies that the collection's `latest_id` matches the newly created artifact after the upsert; failures in this verification path will surface as server-side errors and can cause artifact creation to fail even if the artifact row itself was inserted successfully.
...
Artifact keys must only contain lowercase letters, numbers, and dashes (matching the pattern `^[a-z0-9-]*$`). Keys containing uppercase letters, underscores, or other characters will be rejected by server-side validation before the artifact is created.
