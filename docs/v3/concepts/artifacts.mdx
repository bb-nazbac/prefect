...
When an artifact is created with a non-null `key`, the server automatically inserts or updates an `ArtifactCollection` entry to track this artifact as the latest version for that key. The server enforces a strict consistency check: after the upsert, `ArtifactCollection.latest_id` must equal the newly created artifact's ID, or an error is raised. This means each key has a "latest" pointer that is updated whenever a new artifact is created with that key.
...
Artifact keys must contain only lowercase letters, numbers, and dashes. Keys containing other characters are rejected by server-side validation.

### Creating artifacts from result dicts

When creating an artifact from a result dictionary using `Artifact.from_result`, the reserved keys `artifact_key`, `artifact_type`, and `artifact_description` are extracted from the dictionary and used to populate the corresponding artifact fields. These keys are removed from the input dictionary, so callers reusing the same dict afterward may find those keys missing.

For more informationon how to create and use artifacts, see the [how to produce workflow artifacts](/v3/how-to-guides/workflows/artifacts/) guide.
