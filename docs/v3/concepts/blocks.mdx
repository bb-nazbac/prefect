...
Block schema registration is idempotent by checksum: if a schema with the same fields checksum and version already exists, the server returns the existing schema (HTTP 200) instead of creating a duplicate.
...
Block document data is encrypted by the server before storage, so data is encrypted at rest rather than stored as plaintext.

When creating or updating a block document, data fields containing block references in the form `{"$ref": {"block_document_id": "<id>"}}` are automatically extracted and stored as block document reference records that link the parent document to the referenced document.

When a block document is updated, its block document references are reconciled automatically: new references are created, unchanged references are preserved, and references that are no longer present in the data are deleted.
...
During server startup, Prefect automatically installs a set of protected system block types: **Webhook**, **Secret**, and **LocalFileSystem**. These block types are marked as protected (`is_protected=True`), which means they cannot be deleted via the API. Protected block types can still be updated. The installation uses upsert semantics, so calling the installation endpoint multiple times is idempotent.
...
Protected block types (those with `is_protected=True`) cannot be _deleted_ via the API. However, updates to protected block types are not blocked by the server—the `is_protected` flag only prevents deletion.

The `install_system_block_types` endpoint registers a predefined set of system block types (Webhook, Secret, LocalFileSystem) as protected and uses upsert semantics, so calling it is idempotent.

### Block schema idempotency

Creating a block schema is idempotent by checksum: if a schema with the same fields checksum and version already exists, the existing schema is returned with HTTP 200 instead of creating a new record (which would return HTTP 201).

### Block document data encryption

Block document data is encrypted by the server before storage (data-at-rest encryption) rather than stored as plaintext.

### Block document references

Data fields of the form `{"$ref": {"block_document_id": <id>}}` are extracted during block document creation and stored as `BlockDocumentReference` records linking parent and child documents. The `$ref` entries are removed from the stored data. If a `$ref` field lacks a `block_document_id`, a `ValueError` is raised.

### Recursive resolution of nested block documents

Querying block documents automatically resolves and inlines referenced block documents. The server performs a recursive fetch using stored `$ref` references and reconstructs the full nested data structure before returning results. This means a single query for a block document that references other block documents will return the complete, fully-hydrated document tree.

### Secret field obfuscation

The `include_secrets` parameter (default `False`) controls whether secret fields are revealed or obfuscated when returning block documents. When `include_secrets` is `False`, the server obfuscates secret field values—including those matched by wildcard secret paths in nested structures—before returning the document. To retrieve actual secret values, callers must explicitly set `include_secrets` to `True`.

### Secret preservation on updates

When updating a block document, if a secret field value matches the obfuscated representation of the current secret, the server assumes the client intended to preserve the existing secret and restores the original value instead of overwriting it with obfuscated text. This protection also applies to wildcard secret paths in nested structures, preventing accidental secret erasure when clients post back obfuscated data.

### Reference reconciliation on updates

Updating a block document's data reconciles block document references: new references are created, unchanged ones are preserved, and removed references are deleted automatically. This means modifying a block document's data can cause related `BlockDocumentReference` rows to be created or deleted as a side effect.
