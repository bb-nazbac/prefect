...
If a flow run already exists for the given `flow_id` and `idempotency_key`, the server returns that run and does not apply the state provided in the create request. Initial state is only applied when the run is actually created by the current request.

### Concurrency slot cleanup on deletion

Deleting a flow run associated with a deployment will release any allocated concurrency slots for that deployment, decrementing active slot counters. This ensures that concurrency limits are properly maintained even when flow runs are manually deleted.

### Updating job variables

Updating `job_variables` on a flow run requires the run to be in the `SCHEDULED` state and to have an associated deployment. Job variables are validated against the deployment's work pool schema, and updates are rejected otherwise.

### Resuming a paused flow run
...
Bulk delete operations are capped at 50 flow runs per request. If more runs match the filter, only the first 50 are deleted and each deleted run's logs are scheduled for asynchronous cleanup. Callers must repeat requests to delete beyond the cap.
