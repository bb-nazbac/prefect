...
Asset keys are validated by first checking for restricted characters (such as whitespace, `#`, `?`, `&`, `%`, quotes, angle brackets, brackets, braces, `|`, `\`, `^`, and backticks), then enforcing a maximum length of 512 characters, and finally validating that the key conforms to URI format (e.g., `s3://bucket/path`).
