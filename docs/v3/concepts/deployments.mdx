...
### Deployment upsert and scheduled runs

Creating or upserting a deployment will delete any future auto-scheduled flow runs from the previous deployment version so that scheduled runs reflect the new configuration. This means that updating a deployment's schedule or other settings will clear previously auto-scheduled runs and allow the scheduler to recreate them based on the updated configuration.

### Schedule slug requirements on update

If an existing deployment's schedules have slugs, update requests must also include slugs for all provided schedules. Omitting slugs when the existing schedules have them will result in a `422` error to prevent accidental schedule replacement or deletion.
