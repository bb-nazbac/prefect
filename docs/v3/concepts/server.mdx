...
The `GET /api/health` readiness endpoint actively checks database connectivity. If the database is not connectable, the endpoint returns `503 Service Unavailable`. This means readiness depends on database availability, not just process liveness.
