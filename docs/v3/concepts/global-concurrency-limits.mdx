...
The default lease duration is 5 minutes. The `increment-with-lease` endpoint accepts lease durations between 45 and 86,400 seconds (1 day). Custom durations can be supplied to the concurrency context manager.
...
### Deletion does not revoke leases

Deleting a concurrency limit removes the database row but does not revoke existing leases that reference it. Those leases continue to exist until their TTL expires.

### Inactive limits are excluded from acquisition

Only limits with `active=true` are considered when acquiring slots. Inactive limits in the request are silently ignored during acquisition.

### Missing limit names are silently ignored

Concurrency limit names that do not exist are silently ignored during acquisition. Callers must inspect the response to determine which limits actually participated.

### Denied slots tracking

When slot acquisition fails, the server increments `denied_slots` on the active limits in the request. This counter affects subsequent `Retry-After` calculations and decays over time. It is persisted even on failed acquisition attempts.

### Lease duration bounds

The `increment-with-lease` endpoint defaults `lease_duration` to 300 seconds and enforces a range of 45–86,400 seconds. The lease is persisted using the configured lease storage backend (`server.concurrency.lease_storage`), so lease durability and semantics depend on that backend configuration.

### Idempotent lease release

The `decrement-with-lease` endpoint is idempotent: calling it with a missing or already-revoked `lease_id` returns `204 No Content` and performs no slot changes. When releasing a valid lease, the server computes `occupancy_seconds` from the lease lifetime and updates `avg_slot_occupancy_seconds` for affected limits, influencing future wait estimations.

### Non-lease increment failures

Non-lease increment failures (`POST /api/v2/concurrency_limits/increment`) increment `denied_slots` on active limits and may return 423 Locked with a `Retry-After` header derived from denial history, decay parameters, and jitter.

### Rate limit mode validation

When `mode='rate_limit'`, the endpoint rejects limits with `slot_decay_per_second == 0.0` (returns 422). Only limits with decay configured can be used for rate limiting.

### Slot capacity validation

Slot acquisition fails with 422 if the requested number of slots exceeds any active limit's configured capacity.

### Lease renewal

The `renew_lease` endpoint accepts `True`, `False`, or `None` from the lease backend's `renew_lease` method. A `None` return is treated as a legacy response, and the server checks lease existence to determine the outcome. If the lease is missing, the endpoint returns 410 Gone.
