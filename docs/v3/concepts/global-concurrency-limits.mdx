...
For non-lease increment failures, the server may return `423 Locked` with a `Retry-After` header derived from the denial history, slot decay parameters, and jitter.
