...
`POST /api/task_runs` applies different idempotency rules depending on whether `flow_run_id` is provided. When `flow_run_id` is set, the server performs a database-level upsert keyed on (`flow_run_id`, `task_key`, `dynamic_key`) and returns the existing run if one matches. When `flow_run_id` is absent (ad-hoc task runs), the server uses application-level logic to query and potentially insert based on (`task_key`, `dynamic_key`). In both cases, duplicate runs are prevented, but the underlying mechanisms differ.
...
`POST /api/task_runs/{id}/set_state` applies orchestration rules to proposed state transitions by default. The server compiles rules from task and global policies, which can alter, reject, or transform the requested state. Setting `force=True` bypasses most orchestration rules and uses a minimal policy to allow more direct state writes.

### Cache retrieval on state transitions

When a proposed state includes a `cache_key`, the server's cache retrieval orchestration rule checks for a valid cached `COMPLETED` state matching that key. If a matching, non-expired cache entry is found and `state_details.refresh_cache` is not set, the server rejects the `RUNNING` transition and returns the cached completed state (named `Cached`) instead. Set `refresh_cache` in `state_details` to force bypassing cache retrieval.

### WebSocket subscription protocol

The `WS /api/task_runs/subscriptions/scheduled` endpoint uses a custom protocol: clients must send an initial `subscribe` message with `type: "subscribe"`, `keys`, and `client_id`. After the server dispatches a task run, the client must reply with an `ack` message (or `quit` to disconnect gracefully). If a worker fails to acknowledge a dispatched task properly or disconnects during send, the task is requeued into the retry queue and the worker is removed from active tracking.
