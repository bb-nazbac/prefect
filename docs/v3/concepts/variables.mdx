...
- Only contain lowercase alphanumeric characters ([a-z], [0-9]), underscores (_), or dashes (-). Spaces are not allowed.
- Be unique
...
Creating a variable with a name that already exists fails with `409 Conflict`. The `POST` endpoint does not perform an upsert and will not overwrite an existing variable. To update an existing variable, use the `PATCH` endpoint or the SDK's `Variable.set()` with `overwrite=True`.
