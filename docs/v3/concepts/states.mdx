...
The final state of a flow or task run depends on a number of factors; generally speaking there are four categories of terminal states:
- `COMPLETED`: a run in any `COMPLETED` state did not encounter any errors or exceptions and returned successfully
- `CANCELLED`: a run in any `CANCELLED` state did not complete because a user determined that it should not
